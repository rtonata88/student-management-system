...
class ComprehensiveTripLogsFix extends Migration
{
    ...
    public function up()
    {
        ...
        if (!Schema::hasTable('trip_logs')) {
            ...
            return; // Exit early if we created the table
        }

        // If table exists, add missing columns without using after() to avoid dependency issues
        if (Schema::hasTable('trip_logs')) {
            // Add columns one by one to avoid any reference issues
