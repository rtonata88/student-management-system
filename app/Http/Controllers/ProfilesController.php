...
class ProfilesController extends Controller
{
	...
	public function index()
	{
		...
		return view('profiles.index', compact('countries', 'organisations', 'cities', 'fruit_roles', 'teams', 'fruit_levels', 'fruit_stages',  'profiles'));
	}
	...
	public function filter(Request $request)
	{
		$user = Auth::user();
		if($user->hasRole('department leader')){
			$profiles = Profile::with(['sector', 'language', 'country', 'city', 'organization']);
		} else {
			$profiles = Profile::where('team_id', $user->team_id)->with(['sector', 'language', 'country', 'city', 'organization']);
		}
	...
	public function show($slug){
		...
		return view('profiles.show', compact('profile', 'assistants', 'organizations_list'));
	}
	...
	public function edit($slug)
	{
		...
		return view('profiles.edit', compact('profile','languages', 'sectors', 'countries', 'titles', 'gender', 'organisations', 'cities', 'fruit_roles', 'teams', 'fruit_levels', 'fruit_stages', 'maintainers', 'sector_relationships', 'profile_organizations', 'assistants', 'religions'));
	}
	...
	public function store(Request $requests)
	{
		...
		$profile->slug						= $this->generateSlug($requests->fullname." ".$requests->lastname, 0);
		$profile->gender_id 				= $requests->gender_id;
		$profile->bio 						= $requests->bio;
		$profile->photo 					= $profile_photo;
		$profile->sector_id 				= $requests->sector_id;
		$profile->country_id 				= City::find($requests->city_id)->country_id;
	...
	private function generateSlug($string, $counter){
		$slug = str_slug($string."-".$counter);

		$profile = Profile::whereSlug($slug)->first();
		if(count($profile) > 0){
			$this->generateSlug($string, $counter++);
		} else {
			return $slug;
		}
	}
	...
	public function update(Request $requests, $slug)
	{
		...
		if(count($profile->organization_profile)){
			OrganizationProfile::where('profile_id', $profile->id)->delete();
		}
		foreach ($requests->organization as $key1 => $organization) {
			if($organization){
				$organization_profile = new OrganizationProfile;
		...
		if($profile->profile_assistant){
			ProfileAssistant::where('profile_id', $profile->id)->delete();
		}
		...
		$profile->save();
		return redirect()->route('profiles.show', ['slug' => $profile->slug]);
	}
	...
	public function deleteOrganisation(Request $request){
		OrganizationProfile::destroy($request->id);
		return Response::json(['status' => 200]);
	}

	public function deleteAssistant(Request $request){
		ProfileAssistant::destroy($request->id);
		return Response::json(['status' => 200]);
	}
