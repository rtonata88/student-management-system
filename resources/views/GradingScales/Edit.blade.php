                ...
                <div class="card">
                    <div class="card-header" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white;">
                        <h4 class="card-title mb-0">
                            <i class="fa fa-edit"></i> Edit Grading Scale
                        </h4>
                    ...
                    <div class="card-body">
                        <form action="{{ route('grading-scales.update', $gradingScale->id) }}" method="POST">
                            @csrf
                            @method('PUT')
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <select name="module_id" id="module_id" class="form-control @error('module_id') is-invalid @enderror" required>
                                            ...
                                            @foreach($modules as $module)
                                                <option value="{{ $module->id }}" {{ old('module_id', $gradingScale->module_id) == $module->id ? 'selected' : '' }}>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="academic_year_id" id="academic_year_id" class="form-control @error('academic_year_id') is-invalid @enderror" required>
                                            ...
                                            @foreach($academicYears as $year)
                                                <option value="{{ $year->id }}" {{ old('academic_year_id', $gradingScale->academic_year_id) == $year->id ? 'selected' : '' }}>
                                                    {{ $year->academic_year }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="examination_id" id="examination_id" class="form-control @error('examination_id') is-invalid @enderror" required>
                                            ...
                                            @foreach($examinations as $exam)
                                                <option value="{{ $exam->id }}" {{ old('examination_id', $gradingScale->examination_id) == $exam->id ? 'selected' : '' }}>
                                                    {{ $exam->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                        ...
                                        <select name="result_code_id" id="result_code_id" class="form-control @error('result_code_id') is-invalid @enderror" required onchange="updateGradeFromResultCode()">
                                            ...
                                            @foreach($resultCodes as $resultCode)
                                                <option value="{{ $resultCode->id }}" 
                                                        ...
                                                        {{ old('result_code_id', $gradingScale->result_code_id) == $resultCode->id ? 'selected' : '' }}>
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-bordered" id="grading-scales-table">
                                            ...
                                            <tbody id="grading-scales-tbody">
                                                @foreach($allGradingScales as $index => $scale)
                                                <tr class="grading-scale-row">
                                                    <td>
                                                        <input type="number" name="grading_scales[{{ $index }}][min_mark]" class="form-control min-mark" 
                                                               value="{{ old('grading_scales.'.$index.'.min_mark', $scale->min_mark) }}"
                                                               step="0.01" min="0" max="100" required>
                                                        <input type="hidden" name="grading_scales[{{ $index }}][id]" value="{{ $scale->id }}">
                                                    </td>
                                                    <td>
                                                        <input type="number" name="grading_scales[{{ $index }}][max_mark]" class="form-control max-mark" 
                                                               value="{{ old('grading_scales.'.$index.'.max_mark', $scale->max_mark) }}"
                                                               step="0.01" min="0" max="100" required>
                                                    </td>
                                                    <td>
                                                        <select name="grading_scales[{{ $index }}][result_code_id]" class="form-control result-code-select" required onchange="updateRowFromResultCode(this)">
                                                            <option value="">Select Result Code</option>
                                                            @foreach($resultCodes as $resultCode)
                                                                <option value="{{ $resultCode->id }}" 
                                                                        data-name="{{ $resultCode->name }}" 
                                                                        data-description="{{ $resultCode->description }}"
                                                                        data-pass-fail="{{ $resultCode->pass_fail }}"
                                                                        {{ old('grading_scales.'.$index.'.result_code_id', $scale->result_code_id) == $resultCode->id ? 'selected' : '' }}>
                                                                    {{ $resultCode->name }} ({{ $resultCode->code }})
                                                                </option>
                                                            @endforeach
                                                        </select>
                                                    ...
                                                    <td>
                                                        <input type="text" name="grading_scales[{{ $index }}][grade]" class="form-control grade-input" 
                                                               value="{{ old('grading_scales.'.$index.'.grade', $scale->grade) }}"
                                                               maxlength="255" required placeholder="Auto-populated">
                                                    </td>
                                                    <td>
                                                        <select name="grading_scales[{{ $index }}][pass_fail]" class="form-control pass-fail-select" required>
                                                            <option value="">Auto-populated</option>
                                                            <option value="Pass" {{ old('grading_scales.'.$index.'.pass_fail', $scale->pass_fail) == 'Pass' ? 'selected' : '' }}>Pass</option>
                                                            <option value="Fail" {{ old('grading_scales.'.$index.'.pass_fail', $scale->pass_fail) == 'Fail' ? 'selected' : '' }}>Fail</option>
                                                        </select>
                                                    </td>
                                                    <td>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <div class="form-check">
                                            <input type="checkbox" name="active" id="active" class="form-check-input" value="1" {{ old('active', $gradingScale->active) ? 'checked' : '' }}>
                            ...
                            <div class="form-group mt-4">
                                <button type="submit" class="btn" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; border: none;">
                                    <i class="fa fa-save"></i> Update Grading Scale
                                </button>
...
@section('scripts')
<script>
let rowIndex = {{ count($allGradingScales) }};
