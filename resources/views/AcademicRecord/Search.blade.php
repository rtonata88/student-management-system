            ...
            <div class="card" style="background: linear-gradient(135deg, rgba(255,255,255,0.1), rgba(255,255,255,0.05)); backdrop-filter: blur(10px); border: 1px solid rgba(255,255,255,0.18);">
                <div class="card-header" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white;">
                    <h4 class="card-title mb-0">
                        <i class="fa fa-graduation-cap"></i> Academic Records Search
                    </h4>
                </div>
                <div class="card-body">
                    <!-- Search Form -->
                    <form method="POST" action="{{ route('academic-records.filter') }}">
                        @csrf
                        <div class="row">
                            <div class="col-md-6">
                        ...
                        <div class="row">
                            <div class="col-md-12 text-center">
                                <button type="submit" class="btn btn-lg" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; border: none; border-radius: 25px; padding: 12px 30px; box-shadow: 0 4px 15px rgba(102, 126, 234, 0.4);">
                                    <i class="fa fa-search"></i> Search Students
                                </button>
                                <a href="{{ route('academic-records.index') }}" class="btn btn-lg btn-secondary ml-2" style="border-radius: 25px; padding: 12px 30px;">
                    ...
                    @if(isset($students) && $students->count() > 0)
                        <div class="mt-4">
                            <h5 class="font-weight-bold mb-3">
                                <i class="fa fa-users"></i> Search Results ({{ $students->count() }} student{{ $students->count() > 1 ? 's' : '' }} found)
                            </h5>
                            <div class="row">
                                        ...
                                        <div class="card h-100" style="background: linear-gradient(135deg, rgba(255,255,255,0.9), rgba(255,255,255,0.7)); backdrop-filter: blur(5px); border: 1px solid rgba(255,255,255,0.3); border-radius: 15px; box-shadow: 0 8px 32px rgba(0,0,0,0.1);">
                                            <div class="card-body">
                                                <div class="row">
                                                    ...
                                                    <div class="col-8">
                                                        ...
                                                        <p class="text-muted mb-2 small">
                                                            <strong>Modules:</strong> {{ $student->registered_modules->count() }}
                                                        </p>
                                                    </div>
                                                </div>
                                                ...
                                                @if($student->registered_modules->count() > 0)
                                                    <div class="mt-3">
                                                        @can('generate-academic-records')
                                                            <a href="{{ route('academic-records.generate', $student->id) }}" 
                                                               class="btn btn-sm btn-block" 
                                                               style="background: linear-gradient(135deg, #11998e 0%, #38ef7d 100%); color: white; border: none; border-radius: 20px; padding: 8px 16px; box-shadow: 0 4px 15px rgba(17, 153, 142, 0.4);">
                                                                <i class="fa fa-file-text"></i> View Academic Record
                                                            </a>
                                                        @endcan
                                                        
                                                        @can('download-academic-records')
                                                            <a href="{{ route('academic-records.download', $student->id) }}" 
                                                ...
                                                @else
                                                    <div class="mt-3">
                                                        <span class="badge badge-warning">No registered modules</span>
                                                    </div>
                                                @endif
                                            </div>
                                        </div>
