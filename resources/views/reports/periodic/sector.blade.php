    ...
    <div class="row bg-title">
        ...
        <div class="row">
            <div class="col-md-12 col-lg-12 col-sm-12">
                ...
                <hr>
                @if(count($team_report) > 0 || count($media_coverage_report)>0 || count($events) > 0)
                <span class="box-title"><strong>CALL REPORTS</strong></span>
                @forelse($team_report->where('Activity', 'Call') as $activity)
                ...
                @endif  
                @empty
                No calls were received or made during the selected period
                @endforelse
                <br>
                <span class="box-title"><strong>EMAIL REPORTS</strong></span>
                ...
                <br>
                <span class="box-title"><strong>MEETING REPORTS</strong></span>
                @forelse($team_report->where('Activity', 'Meeting') as $activity)
               ...
               <?php $activities = \App\ActivityTeamReport::get_activities($activity->sector_id, $activity->team_id, $activity->activity_type_id, $start_date, $end_date)?>
                ...
                @endif  
                @empty
                No meetings were recorded during the selected period
                @endforelse

                <br>
                <span class="box-title"><strong>MEDIA COVERAGES</strong></span>
                @forelse($media_coverage_report as $activity)
                @if($activity)
               <?php $activities = \App\ActivityTeamReport::get_media_coverage($activity->sector_id, $activity->team_id, $start_date, $end_date)?>
                ...
                @endif  
                @empty
                No media coverage was covered during the selected period
                @endforelse
                <br>
                <span class="box-title"><strong>EVENTS</strong></span>

                @forelse($events as $activity1)
                @if($activity1)
               <?php $activities = \App\ActivityTeamReport::get_events($activity1->sector_id, $activity1->team_id, $start_date, $end_date)?>
                ...
                @endif  
                @empty
                No events hosted or attended during the selected period
                @endforelse
