...
<div class="c-subheader px-3">
    ...
    <ol class="breadcrumb border-0 m-0">
        ...
        <li class="breadcrumb-item active"><a href="/exam-permits">Exam Permits</a></li>
        <!-- Breadcrumb Menu-->
    </ol>
</div>
@endsection
        ...
        <div class="search-card">
            <div class="search-header">
                <h4 class="search-title">
                    ...
                    Find Student for Exam Permit
                </h4>
                <p class="search-subtitle">Search by student number or name to generate examination permit</p>
            </div>
            
            {!! Form::open(array('route' => array('exam-permits.filter'), 'method' => 'post', 'class'=> 'search-form')) !!}
    ...
    <div class="row">
        ...
        <div class="card">
            <div class="card-header">
                <strong> Select student to generate exam permit </strong>
            </div>
            <div class="card-body">
                        ...
                        <tr>
                            ...
                            <td>
                                @if($student->registered_modules->count() > 0)
                                    <span class="badge bg-success">{{ $student->registered_modules->count() }} subjects</span>
                                @else
                            ...
                            <td>
                                <div class="d-flex gap-2 align-items-center">
                                    @if($student->registered_modules->count() > 0)
                                        <a href="{{route('exam-permits.generate', $student->id)}}" class="btn btn-sm" 
                                           style="background: linear-gradient(135deg, #6f42c1 0%, #007bff 100%); color: white; border: none; border-radius: 6px; padding: 0.375rem 0.75rem;">
                                            <i class="fas fa-file-alt me-1"></i>Generate Permit
                                        </a>
                                    @else
                                        <span class="text-muted small">No subjects registered</span>
                                    @endif
                                </div>
                            </td>
                        </tr>
